<x-workflow-layout>
    <x-slot name="header">
        <h1 class="font-display font-medium text-[32px] tracking-wide text-vellum">Edit Department</h1>
        <div class="text-[13px] text-vellum-muted mt-1.5 tracking-wide">
            Update properties for department group {{ $department->name }}
        </div>
    </x-slot>
            ...
            <div class="grid grid-cols-1 gap-6 mb-6">
                ...
                <div>
                    ...
                    <textarea
                        ...
                        name="description"
                        rows="4"
                        class="w-full bg-surface-raised border border-hairline rounded-md text-vellum px-3 py-2 text-sm focus:ring-1 focus:ring-brass focus:border-brass focus:outline-none mt-1.5">{{ old('description', $department->description) }}</textarea>
                    @error('description')
                        <p class="text-burgundy font-mono text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
